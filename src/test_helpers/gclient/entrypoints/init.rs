use crate::test_helpers::{gclient::utils::ProgramId, consts::INVARIANT_PATH};
use gclient::{EventListener, EventProcessor, GearApi};
use gstd::prelude::*;
use io::*;

pub async fn init_invariant(api: &GearApi, listener: &mut EventListener, init: InitInvariant) -> ProgramId {
  let init_payload = init.encode();

  let gas_info = api
      .calculate_upload_gas(
          None,
          gclient::code_from_os(INVARIANT_PATH).expect("failed to get code"),
          init_payload.clone(),
          0,
          true,
      )
